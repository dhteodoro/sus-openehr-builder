...
/**
 * JSON serializer of the openEHR Archetype Object Model.
 * 
 * @author Douglas Teodoro
 */
public class JSONSerializer {

	public static final Namespace defaultNamespace = Namespace
	...
	public static final Namespace xsdNamespace = Namespace.getNamespace("xsd",
			"http://www.w3.org/2001/XMLSchema");

	public static final Namespace xsiNamespace = Namespace.getNamespace("xsi",
			"http://www.w3.org/2001/XMLSchema-instance");

	/* fields */
	private Charset encoding;

	private ObjectMapper outputter;
	...
	protected void printAssertion(Assertion assertion, Element out) {
		...
		if (assertion.getTag() != null) { // must not be displayed at all if
											// null according to spec.
	...
	protected void printCAttribute(CAttribute cattribute, Element out) {
		...
		if (isMultipleAttribute) {
			attributes.setAttribute("type", "C_MULTIPLE_ATTRIBUTE",
					xsiNamespace);
		} else {
			attributes.setAttribute("type", "C_SINGLE_ATTRIBUTE", xsiNamespace);
		}

		// FIXME: AOM XML schema spec is wrong. Has 'unbounded' attribute for
		// C_ATTRIBUTE and element with name
		// 'rm_type_name' instead of 'rm_attribute_name'.
		printString("rm_attribute_name", cattribute.getRmAttributeName(),
				attributes);
		...
		if (cattribute.getExistence().equals(CAttribute.Existence.REQUIRED)) {
			lower = 1;
			upper = 1;
		} else if (cattribute.getExistence().equals(
	...
	protected void printCBoolean(CBoolean cboolean, Element out) {
		...
		printString("true_valid", cboolean.isTrueValid() == true ? "true"
				: "false", out);
		printString("false_valid", cboolean.isFalseValid() == true ? "true"
				: "false", out);

		if (cboolean.hasAssumedValue()) {
			printString("assumed_value",
					cboolean.assumedValue().booleanValue() == true ? "true"
							: "false", out);
		}
	}
	...
	protected void printCCodePhrase(CCodePhrase ccp, Element out) {
		...
		if (ccp.getTerminologyId() != null) {
			Element terminologyId = new Element("terminology_id",
					defaultNamespace);
			children.getChildren().add(terminologyId);
			printString("value", ccp.getTerminologyId().getValue(),
					terminologyId);
		}
	...
	protected void printCDateTime(CDateTime cdatetime, Element out) {
		...
		// FIXME: Output timezone_validity. CDateTime seem to be missing this
		// function.
		...
		if (cdatetime.hasAssumedValue()) {
			printString("assumed_value", cdatetime.assumedValue().toString(),
					out);
		}
	}
	...
	protected void printCDomainType(CDomainType cdomain, Element out) {
		...
		if (cdomain instanceof CCodePhrase) {
			...
			// unknown CDomainType
			System.err.println("Cannot serialize CDomainType of type '"
					+ cdomain.getClass().getName() + "'!");
		}
	}
	...
	protected void printCDuration(CDuration cduration, Element out) {
		...
		if (cduration.hasAssumedValue()) {
			printString("assumed_value", cduration.assumedValue().toString(),
					out);
		}
	}
	...
	protected void printCDvOrdinal(CDvOrdinal cordinal, Element out) {
		...
		if (cordinal.getList() != null) {
			...
			for (Iterator<Ordinal> it = ordinals.iterator(); it.hasNext();) {
				...
				Element definingCode = new Element("defining_code",
						defaultNamespace);
		...
		if (cquantity.getList() != null) {
			...
			for (CDvQuantityItem item : list) {
				...
				if (item.getMagnitude() != null) {
					Element magnitude = new Element("magnitude",
							defaultNamespace);
	...
	protected void printCInteger(CInteger cinteger, Element out) {
		...
		// TODO: Write 'list_open' element... <list_open> xs:boolean
		// </list_open> [0..1]

		if (cinteger.hasAssumedValue()) {
			printString("assumed_value", cinteger.assumedValue().toString(),
					out);
		}
	}
	...
	protected void printCObject(CObject cobj, Element out) {
		...
		} else if (cobj instanceof ConstraintRef) { // FIXME: Add in
													// ADLSerializer as well
	...
	protected void printCObjectElements(CObject cobj, Element out) {
		...
		// we always need the upper case with underscore notation for the rm
		// type name
		printString("rm_type_name",
				getUpperCaseWithUnderscoreFromCamelCase(cobj.getRmTypeName()),
				out);
	...
	protected void printCPrimitive(CPrimitive cp, Element out) {
		...
		if (cp instanceof CBoolean) {
			...
			System.err.println("Cannot serialize CPrimitive of type '"
					+ cp.getClass().getName() + "'!");
		}
	...
	protected void printCTime(CTime ctime, Element out) {
		...
		// FIXME: Output timezone_validity. CTime seem to be missing this
		// function.
	...
	protected void printDescription(ResourceDescription description, Element out) {
		...
		printStringList("other_contributors",
				description.getOtherContributors(), des);
		printString("lifecycle_state", description.getLifecycleState(), des);

		if (description.getResourcePackageUri() != null
				&& description.getResourcePackageUri().length() > 0) { // only
																		// add
																		// this
																		// tag
																		// if
																		// non-empty
			printString("resource_package_uri",
					description.getResourcePackageUri(), des);
		}
	...
	protected void printDescriptionItem(ResourceDescriptionItem item,
		...
		printStringMap("original_resource_uri", item.getOriginalResourceUri(),
				out);

		printStringMap("other_details", item.getOtherDetails(), out);
	}

	protected void printExpressionItem(String label, ExpressionItem expItem,
			Element out) {
	...
	protected void printExpressionLeaf(String label, ExpressionLeaf expLeaf,
			Element out) {
		...
		if (expLeaf.getItem() instanceof CPrimitive) {
			...
			// EXPR_LEAF.item is defined as xs:any in the AM schema (the reason
			// for which is unclear)
			// and consequently of type object in C# land.
			// That's why having the XSD type explicitly set to string fixes the
			// problem when deserialising to a C# string instead of XmlText.
			printString("item", expLeaf.getItem().toString(), elm, true);
		}
		printString("reference_type", expLeaf.getReferenceType().name()
				.toLowerCase(), elm); // the Reference type is expected in lower
										// case here as per spec examples
	}

	protected void printExpressionOperator(String label,
			ExpressionOperator expOperator, Element out) {
		Element elm = new Element(label, defaultNamespace);
		out.getChildren().add(elm);
		if (expOperator instanceof ExpressionUnaryOperator) {
			...
			printExpressionItem("operand",
					((ExpressionUnaryOperator) expOperator).getOperand(), elm);
			...
			printExpressionOperatorElements(expOperator, elm);
			printExpressionItem("left_operand",
					expBinaryOperator.getLeftOperand(), elm);
			printExpressionItem("right_operand",
					expBinaryOperator.getRightOperand(), elm);
		} else {
	...
	protected void printExpressionOperatorElements(
			ExpressionOperator expOperator, Element out) {

		printString("type", expOperator.getType(), out);
		printString("operator",
				String.valueOf(expOperator.getOperator().getValue()), out);
		printString(
				"precedence_overridden",
				expOperator.isPrecedenceOverridden() == true ? "true" : "false",
				out);
	}

	protected void printHeader(Archetype archetype, Element out) {
		Element originalLanguage = new Element("original_language",
				defaultNamespace);
		out.getChildren().add(originalLanguage);
		printCodePhrase(archetype.getOriginalLanguage(), originalLanguage);
		printString("is_controlled", archetype.isControlled() ? "true"
				: "false", out);
		...
		if (parentID != null) {
			Element parentArchetypeId = new Element("parent_archetype_id",
					defaultNamespace);
	...
	protected void printInterval(Interval interval, Element out) {
		...
		if (!interval.isLowerUnbounded()) { // not included is implied if
											// unbounded
			printString("lower_included",
					interval.isLowerIncluded() == true ? "true" : "false", out);
		}
		if (!interval.isUpperUnbounded()) { // not included is implied if
											// unbounded
			printString("upper_included",
					interval.isUpperIncluded() == true ? "true" : "false", out);
		}

		printString("lower_unbounded", interval.isLowerUnbounded() ? "true"
				: "false", out);

		printString("upper_unbounded", interval.isUpperUnbounded() ? "true"
				: "false", out);
	...
	protected void printOntology(ArchetypeOntology ontology, String concept,
			Element out) {
		...
		// TODO: Check why this is not in the XML schema specification of the
		// AOM.
		// printString("primary_language", ontology.getPrimaryLanguage(), 2,
		// out);
		// TODO: Check why this is not in the XML schema specification of the
		// AOM.
		// printStringList("languages_available", ontology.getLanguages(), 2,
		// out);
		// printStringList("terminologies_available",
		// ontology.getTerminologies(), 2, out);
		// final int specDepth = StringUtils.countMatches(".", concept);
		// printString("specialisation_depth", String.valueOf(specDepth), 2,
		// out);

		if (ontology.getTermDefinitionsList() != null) {
			final List<OntologyDefinitions> termDefinitions = ontology
					.getTermDefinitionsList();

			for (OntologyDefinitions defs : termDefinitions) {
				Element trmDefinitions = new Element("term_definitions",
						defaultNamespace);
		...
		if (ontology.getConstraintDefinitionsList() != null) {
			final List<OntologyDefinitions> constraintDefinitions = ontology
					.getConstraintDefinitionsList();

			for (OntologyDefinitions defs : constraintDefinitions) {
				Element consDefinitions = new Element("constraint_definitions",
						defaultNamespace);
		...
		if (ontology.getTermBindingList() != null) {
			final List<OntologyBinding> termBindings = ontology
					.getTermBindingList();

			TermBindingItem item;
			for (OntologyBinding bind : termBindings) {
				Element trmBindings = new Element("term_bindings",
						defaultNamespace);
				...
				for (OntologyBindingItem bindItem : bind.getBindingList()) {
					...
					for (String value : item.getTerms()) {
						...
						String terminologyId = value.substring(1,
								value.lastIndexOf("::"));
						String codeString = value
								.substring(value.lastIndexOf("::") + 2,
										value.length() - 1);
						Element termId = new Element("terminology_id",
								defaultNamespace);
		...
		if (ontology.getConstraintBindingList() != null) {
			final List<OntologyBinding> constraintBindings = ontology
					.getConstraintBindingList();

			QueryBindingItem item;
			for (OntologyBinding bind : constraintBindings) {
				Element consBindings = new Element("constraint_bindings",
						defaultNamespace);
 ...
 * 
 * The Original Code is JSONSerializer.
 * 
 * The Initial Developer of the Original Code is Universidade do Estado do Rio
 * de Janeiro, Brazil. Portions created by the Initial Developer are Copyright
 * (C) 2005-2008 the Initial Developer. All Rights Reserved.
 * 
 * Contributor(s): Douglas Teodoro <douglas@lampada.uerj.br>
 * 
